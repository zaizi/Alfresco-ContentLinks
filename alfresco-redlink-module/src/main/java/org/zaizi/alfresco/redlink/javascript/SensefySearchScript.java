 ...
 */
package org.zaizi.alfresco.redlink.javascript;
...
import org.zaizi.alfresco.redlink.domain.AlfrescoFacet;
import org.zaizi.alfresco.redlink.model.SensefyModel;
import org.zaizi.alfresco.redlink.service.search.solr.SensefySolrJSONResultSet;
import org.zaizi.alfresco.redlink.service.search.solr.SensefySolrQueryHTTPClient;
import org.zaizi.alfresco.redlink.util.EntityTypeExtractor;

/**
 * Search component for use by the ScriptService Modified
 * 
 * @author aayala
 * @author Antonio David Perez Morales <aperez@zaizi.com>
 */
public class SensefySearchScript extends BaseScopableProcessorExtension
{
    private static Log logger = LogFactory.getLog(SensefySearchScript.class);
    ...
    private SensefySolrQueryHTTPClient sensefySolrQueryHttpClient;
        ...
        if (search != null && search.length() != 0)
        {
            ...
            try
            {
                ...
                List<NodeRef> nodes = this.services.getSearchService().selectNodes(
                        nodeService.getRootNode(new StoreRef(store)), search, null,
    ...
    public Scriptable savedSearch(ScriptNode savedSearch)
    {
        ...
        try
        {
            if (savedSearch != null)
            {
                ContentReader content = this.services.getContentService().getReader(savedSearch.getNodeRef(),
    ...
    public Scriptable query(Object search)
    {
        ...
        if (search instanceof Serializable)
        {
            ...
            if (obj instanceof Map)
            {
                ...
                // facet
                List<String> facets = (List<String>) def.get("facets");

                // filters
                List<String> filters = new ArrayList<String>();
                if (def.containsKey("filters"))
                {
                    filters = (List<String>) def.get("filters");
                ...
                if (facets != null && !facets.isEmpty())
                {
                    ...
                    {
                        String fullFacetName = this.toFullyQualifiedName(facet);
                        FieldFacet facetField = new FieldFacet(fullFacetName);
                        facetField.setMinCount(1);
                        sp.addFieldFacet(facetField);
                    }
                }
                ...
                if (onerror != null)
                {
                    ...
                    {
                        throw new AlfrescoRuntimeException("Failed to search: Unknown value supplied for 'onerror': "
                                + onerror);
                    }
                }
    ...
    /**
     * <p>Convert a property (fully qualified name or not) in a fully qualified name property starting with @ to be used in search queries</p>
     * @param property The property to be transformed
     * @return the fully qualified name of the property starting with @
     */
    private String toFullyQualifiedName(String property) {
       String[] splitted = QName.splitPrefixedQName(property);
       String fqn;
       if(splitted[0].equals(NamespaceService.DEFAULT_PREFIX)) {
           fqn = property.startsWith("@") ? property : "@"+property;
       }
       else {
           fqn = "@"+QName.createQName(splitted[0], splitted[1], this.services.getNamespaceService()).toString();
       }
     ...
     * @return Array of Node objects
     */
    protected Object[] query(String store, String search, SortColumn[] sort, String language, int maxResults,
            int skipResults)
    {
        SearchParameters sp = new SearchParameters();
        sp.addStore(store != null ? new StoreRef(store) : this.storeRef);
    ...
    protected Object[] query(SearchParameters sp, boolean exceptionOnError)
    {
        ...
        Collection<AlfrescoFacet> facets = null;

        if (logger.isDebugEnabled())
            logger.debug("query=" + sp.getQuery() + " limit="
        ...
        try
        {
            ...
            //results = sensefySolrQueryHttpClient.executeQuery(sp);
            ...
            // facets
            /*if (results instanceof SensefySolrJSONResultSet)
            {
                SensefySolrJSONResultSet solrResults = (SensefySolrJSONResultSet) results;*/

                facets = new LinkedHashSet<AlfrescoFacet>();
                for (FieldFacet facet : sp.getFieldFacets())
                {
                    List<Pair<String, Integer>> facetValues = results.getFieldFacet(facet.getField());
                    if (!facetValues.isEmpty())
                    {
                        // custom behaviour for categories
                        if (facet.getField().equals(
                                ContentModel.PROP_CATEGORIES.toPrefixString(services.getNamespaceService())))
                        {
                            Map<String, AlfrescoFacet> mapFacets = new HashMap<String, AlfrescoFacet>();
                            Iterator<Pair<String, Integer>> it = facetValues.iterator();
                            while (it.hasNext())
                            {
                                Pair<String, Integer> par = it.next();
                                NodeRef catRef = new NodeRef(par.getFirst());
                                if (services.getNodeService().exists(catRef)
                                        && services.getNodeService().hasAspect(catRef,
                                                SensefyModel.ASPECT_URIABLE))
                                {
                                    String type = entityTypeExtractor.getType((List<String>) services.getNodeService().getProperty(
                                            catRef, SensefyModel.PROP_TYPES));
                                    if (type != null)
                                    {
                                        AlfrescoFacet fac = null;
                                        if (mapFacets.containsKey(type))
                                        {
                                            fac = mapFacets.get(type);
                                        }
                                        else
                                        {
                                            fac = new AlfrescoFacet();
                                            // fac.setName("@" + ContentModel.PROP_CATEGORIES + "||" + type);
                                            fac.setName(ContentModel.PROP_CATEGORIES.toPrefixString(services.getNamespaceService())
                                                    + "||" + type);
                                        }

                                        fac.addFieldValue(par.getFirst(), par.getSecond());
                                        mapFacets.put(type, fac);
                                    }
                                }
                            }

                            // addall
                            facets.addAll(mapFacets.values());
                        }
                        else
                        {
                            AlfrescoFacet fac = new AlfrescoFacet();
                            fac.setName(facet.getField());
                            fac.addAllFacetValues(facetValues);
                            facets.add(fac);
                        }
        ...
        Object[] arrayResults = new Object[2];
        arrayResults[0] = set != null ? set.toArray(new Object[(set.size())]) : new Object[0];
        arrayResults[1] = facets != null ? facets.toArray(new Object[facets.size()]) : new Object[0];
        return arrayResults;
    }
    ...
    public SensefySolrQueryHTTPClient getSensefySolrQueryHttpClient()
    {
        return sensefySolrQueryHttpClient;
    }

    public void setSensefySolrQueryHttpClient(SensefySolrQueryHTTPClient sensefySolrQueryHttpClient)
    {
        this.sensefySolrQueryHttpClient = sensefySolrQueryHttpClient;
    }
